...
def make_greyscale(input_image):
    ...
    decode_array_toimage = cv2.imdecode(image_array,cv2.IMREAD_UNCHANGED)
    print('decode values for image : ', decode_array_toimage)
    converted_grayimg= cv2.cvtColor(decode_array_toimage,cv2.COLOR_RGB2GRAY)
    status,output_image = cv2.imencode('.PNG', converted_grayimg)
    print('status:' , status)
    return output_image
